...
function addCustomerName() {
    ...
    let h1tag = document.getElementById('tm-header')
    h1tag.textContent = customerName;

    // <h2 id="introheader" class="tm-section-title mb-5 text-uppercase tm-color-primary"></h2> <!--todo: Add Customer Name-->
    
    let h2tag = document.getElementById('introheader')
    h2tag.textContent = customerName;
};

function toggleColors() {
    // <!-- todo: Change all classes named 'tm-white-rect' to 'tm-blue-rect' -->
    let togColor = document.getElementsByClassName('tm-white-rect')
    for (i = 0; i < togColor.length; ++i) {
        togColor[i].classList.add('tm-blue-rect');
        togColor[i].classList.remove('tm-white-rect');
    }
}
function hideElement() {
    //<ul class="nav flex-row tm-social-links">  <!--todo: Hide social links-->
    let sLink = document.querySelector('.tm-social-links');
    sLink.style.visibility = "none";
};
